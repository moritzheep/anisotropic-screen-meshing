...
namespace pmp
{
    ...
    public:
        ...
        static constexpr Scalar sigma_ = std::sqrt(2);
    ...
    private:
        ...
        void calculate_curvature()
        {
            ...
            max_abs_curvatures_ = cv::Mat(height, width, CV_32FC1);
            ...
            cv::Mat blurredNormals;

            // Apply blur before taking derivatives
            cv::GaussianBlur(normals_, blurredNormals, cv::Size(5, 5), sigma_);

            // Renormalize blurred normals
            for (int v = 0; v != height; ++v)
            {
                for (int u = 0; u != width; ++u)
                {
                    normal = normalize(Normal(blurredNormals.at<cv::Vec3f>(v, u)[0] - std::numeric_limits<Scalar>::epsilon(), blurredNormals.at<cv::Vec3f>(v, u)[1], blurredNormals.at<cv::Vec3f>(v, u)[2]));
                    blurredNormals.at<cv::Vec3f>(v, u) = cv::Vec3f(normal[0], normal[1], normal[2]);
                }
            }
            ...
            for (int v = 0; v != height; ++v)
            {
                for (int u = 0; u != width; ++u)
                {
                    ...
                    if (mask_.empty() || ((mask_.at<uchar>(v, u) > 127) && (mask_.at<uchar>(vp, hp) > 127) && (mask_.at<uchar>(vm, hp) > 127) && (mask_.at<uchar>(vp, hm) > 127) && (mask_.at<uchar>(vm, hm) > 127)))
                    {
                        ...
                        // Remember BGR to RGB
                        normal = Normal(blurredNormals.at<cv::Vec3f>(v, u)[2], blurredNormals.at<cv::Vec3f>(v, u)[1], blurredNormals.at<cv::Vec3f>(v, u)[0]);

                        auto t = tangents(normal, pmp::Point(u, v, 1));

                        dX.col(0) = Eigen::Vector<Scalar, 3>(t.first);
                        dX.col(1) = Eigen::Vector<Scalar, 3>(t.second);

                        for (int i = 0; i != 3; ++i)
                        {
                            // Remember BGR to RGB!
                            dN(i, 0) = (blurredNormals.at<cv::Vec3f>(v, hp)[2 - i] - blurredNormals.at<cv::Vec3f>(v, hm)[2 - i]) / static_cast<Scalar>(hp - hm);
                            dN(i, 1) = (blurredNormals.at<cv::Vec3f>(vp, u)[2 - i] - blurredNormals.at<cv::Vec3f>(vm, u)[2 - i]) / static_cast<Scalar>(vp - vm);
                        }
                        ...
                        max_abs_curvatures_.at<float>(v, u) = solver.eigenvalues().array().abs().matrix().maxCoeff();
                    ...
                    {
                        max_abs_curvatures_.at<float>(v, u) = 0;
                    }
                }
            }
        }
    ...
    public:
        ScreenDifferentialGeometry(SurfaceMesh& mesh, const cv::Mat& normals, const cv::Mat& mask = cv::Mat(), Camera camera = Camera()) : mesh_(mesh), camera_(camera), normals_(normals), mask_(mask)
        {
            calculate_curvature();
        ...
        void update()
        {
            ...
            tmesh.positions.index_put_({ "...", 0 }, 2. * (tmesh.positions.index({ "...", 0 }) - static_cast<float>(umin_) + 0.5) / static_cast<float>(umax_ - umin_) - 1.);
            tmesh.positions.index_put_({ "...", 1 }, 2. * (tmesh.positions.index({ "...", 1 }) - static_cast<float>(vmin_) + 0.5) / static_cast<float>(vmax_ - vmin_) - 1.);
            tmesh.positions.index_put_({ "...", 2 }, 0);

            // Triangle ids give one triangle for each pixel
            Renderer& renderer = Renderer::getInstance();
            torch::Tensor tIds = renderer.render(tmesh.positions, tmesh.triangles, std::make_tuple(vmax_ - vmin_, umax_ - umin_))[0].index({ 0, "...", 3 }).to(torch::kInt).to(torch::kCPU);
            ...
            for (int v = 0; v != vmax_ - vmin_; ++v)
            {
                for (int u = 0; u != umax_ - umin_; ++u)
                {
                    ...
                    if (id > 0 && (mask_.empty() || mask_.at<uchar>(v + vmin_, u + umin_) > 127))
                    {
                        Face f(id - 1);
                        normal = Normal(normals_.at<cv::Vec3f>(v + vmin_, u + umin_)[2], normals_.at<cv::Vec3f>(v + vmin_, u + umin_)[1], normals_.at<cv::Vec3f>(v + vmin_, u + umin_)[0]);
                        ray = camera_.dz(u + umax_, v + vmin_);

                        fnormal_[f] += normal;
                        fmax_abs_curvature_[f] = std::max<Scalar>(max_abs_curvatures_.at<float>(v + vmin_, u + umin_), fmax_abs_curvature_[f]);
                    }
                }
            }
